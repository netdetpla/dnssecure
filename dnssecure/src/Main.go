...
func main() {
	...
	_ = SendUDP(tasks.taskID, randString, "run: " + strconv.FormatInt(startTime, 10))
	//任务执行
	TaskRun()
	...
	_ = SendUDP(tasks.taskID, randString, "len: " + strconv.Itoa(len(tasks.records)) + "; duration: " + strconv.FormatInt(duration, 10))
	//写状态文件
  var (
    //err        error
